    ...
    <li>
      <div class="cart_bottom">
        ...
        <?php foreach ($totals as $total) {
          $prix=str_replace('₪', '', $total['text']);
          
        }?>
          <tr>
            <td class="text-left"><span>الشحن</span></td>
            <td class="text-right"><?php if($prix<150) echo '₪10.00'; else echo 'مجاني'?></td>
          </tr>
          <?php $t=0; 
          foreach ($totals as $total) { ?>
          <tr>
            <td class="text-left"><span>المجموع</span></td>
            <td class="text-right <?php echo ($total==end($totals) ? ' last' : ''); ?>"><?php $prix=str_replace('₪', '', $total['text']); 
              if($prix < 150)
                $prix+=10;
