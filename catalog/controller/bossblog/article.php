		...
		if ($article_info) {
			...
			$results = $this->model_bossblog_article->getArticleRelated($this->request->get['blog_article_id']);
           	...
           	foreach ($results as $result) {
				...
				$data['articles'][] = array(
                    ...
                    'title'    	 => utf8_substr(strip_tags(html_entity_decode($result['title'], ENT_QUOTES, 'UTF-8')), 0, 150),
			...
			foreach ($results as $result) {
				...
				if ($result['image']) {
					...
					if(isset($module_label['product']) && $module_label['product'] == 1){					
						...
						if($checklabel == 1){
							$image = $this->model_bosslabelproducts_bossimage->label_resize($result['image'], $this->config->get('config_bossblog_image_related_width'), $this->config->get('config_bossblog_image_related_height'),$result['product_id']);					
						}else{
							$image = $this->model_tool_image->resize($result['image'], $this->config->get('config_bossblog_image_related_width'), $this->config->get('config_bossblog_image_related_height'));
						}
					}else{
					...
					if(isset($module_label['featured_module_percent']) && $module_label['featured_module_percent'] == 1){
						$price_percent = $this->tax->calculate($result['price'], $result['tax_class_id'], $this->config->get('config_tax'));
						if ((float)$result['special']) {
							$results_special = $this->tax->calculate($result['special'], $result['tax_class_id'], $this->config->get('config_tax'));
						} else {
							$results_special = false;
						}
						if($results_special){
							if($price_percent!=0){
								$percent = round(abs($price_percent-$results_special)/$price_percent*100);	
							}
						}
					}
				} else {
