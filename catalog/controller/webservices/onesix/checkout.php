...
class ControllerWebservicesOneSixCheckout extends Controller {
        ...
        if (!empty($this->request->post['email'])) {
            ...
            if (!empty($check_customer)) {
                ...
                if (isset($address_id)) {
                    $data['shipping_address_reponse'] = array(
                        "status" => "success",
                        "message" => $this->language->get('text_shipping_address_success')
                    );
                } else {
                    $data['shipping_address_reponse'] = array(
                        "status" => "failure",
                        "message" => $this->language->get('text_shipping_address_issue')
                    );
                }
            } else {
                $data['shipping_address_reponse'] = array(
    ...
    public function updateDetailsOfShippingAddress() {
        ...
        if (!empty($this->request->post['id_shipping_address'])) {
            ...
            if (!empty($check_customer)) {
                ...
                $data['shipping_address_reponse'] = array(
                    "status" => "success",
                    "message" => $this->language->get('text_shipping_address_update')
                );
            } else {
                $data['shipping_address_reponse'] = array(
                    "status" => "failure",
    ...
    public function ClearCart() {
        ...
        if (isset($this->request->post['email'])) {
            ...
            $data['clear_cart'] = array('status' => 'failure', 'message' => iconv("UTF-8","UTF-8//IGNORE",$this->language->get('text_error')), 'session_data' => $this->session->data['cart']);
        }
            ...
            if (!$redirect) {
                ...
                foreach ($results as $result) {
                    if ($this->config->get($result['code'] . '_status')) {
                       ...
                       if (VERSION >= '2.2.0.0') {
                            // We have to put the totals in an array so that they pass by reference.
                            $this->{'model_total_' . $result['code']}->getTotal($order_data);
                        } else {
...
//print_R($this->session->data['shipping_address']);die;
                ...
                $data['message'] = iconv("UTF-8","UTF-8//IGNORE",$this->language->get('app_text_order_created'));
