...
class ControllerWebservicesOneSixShoppingCart extends Controller {
    ...
    public function appAddToCart() {
        ...
        if (!empty($this->request->post['cart_products'])) {
            ...
            $json['status'] = 'failure';
            $json['message'] = $this->language->get('text_product_not_added');
            $json['session_data'] = $this->session->getId();
        }
        ...
        if (isset($session_data) && $session_data != '') {
            ...
            if (!empty($this->request->post['cart_products'])) {
               ...
               $quantity = (int) $products['quantity'];
                ...
                if($quantity < $product_detail['minimum']) {
                
                $json = $this->cartDetails();    
                $json['status'] = 'failure';
                $json['message'] = iconv("UTF-8","UTF-8//IGNORE",$this->language->get('error_update_quantity_1'));
                $json['session_data'] = $this->session->getId();
                ...
                } else if($product_detail['quantity']<$quantity) {
                
                $json = $this->cartDetails();    
                $json['status'] = 'failure';
                $json['message'] = iconv("UTF-8","UTF-8//IGNORE",$this->language->get('error_update_quantity_2'));
                $json['session_data'] = $this->session->getId();
...
//                else {
    ...
    public function cartDetails() {
        ...
        if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
            ...
            if (isset($this->request->post['shipping_method']) && $this->request->post['shipping_method'] != '') {
                ...
                if (!$json) {
                    ...
                    if ($this->session->data['shipping_methods']) {
                        ...
                        $data['shipping_warning'] = sprintf($this->language->get('error_no_shipping'), $this->url->link('information/contact','',true));
                    }
                } else {
            ...
            $width = $this->language->get('image_width');
            $height = $this->language->get('image_height');
            ...
            foreach ($products as $product) {
                ...
                if ($product['minimum'] > $product_total) {
                    $error_warning[] = "<b>*" . $product['name'] . ":</b> Minimum <b>" . $product['minimum'] . "</b> quantity required.";
                    //$data['error_warning'] = sprintf($this->language->get('error_minimum'), str_replace('&quot;', '"', $product['name']), $product['minimum']);
                }

                if ($product['image']) {
                    $image = $this->model_webservices_image->resize($this->imageUrlEncoding($product['image']), $width, $height);
                } else {
                    $image = '';
                }
		...
		$this->load->language("checkout/cart");
            ...
            foreach ($totals as $total) {
                ...
                if ($total['code'] == 'voucher') {
                    ...
                    if (version_compare(VERSION, '2.1.0.1', '>=')) {
                        $voucher_info = $this->model_total_voucher->getVoucher($code);
                    } else {
...
//            }
            ...
            $data['cart']['empty'] = $this->language->get('text_empty');
        }
	...
	$data = array();
		...
		$data = $this->cartDetails();
            $data['status'] = 'success';
            $data['message'] = iconv("UTF-8","UTF-8//IGNORE",$this->language->get('text_voucher_already'));
            ...
            if (isset($this->request->post['voucher']) && version_compare(VERSION, '2.1.0.1', '>=')) {
                ...
                $this->load->language('total/coupon');
                $voucher = $this->request->post['voucher'];
                $voucher_info = $this->model_total_coupon->getCoupon($voucher);
                if ($voucher_info) {
            ...
            if ($status) {
                $data['status'] = 'success';
                $data['message'] = iconv("UTF-8","UTF-8//IGNORE",$this->language->get('text_voucher_success'));
            } else {
                $data['status'] = 'failure';
    ...
    public function appRemoveVoucher() {
        ...
        if ($status) {
            $data['status'] = 'success';
            $data['message'] = iconv("UTF-8","UTF-8//IGNORE",$this->language->get('text_voucher_removed'));
        } else {
            $data['status'] = 'failure';
