    ...
    public function appAddToWishlist() {
        ...
        if (!empty($this->request->post['email']) && $this->request->post['product_id']) {
            ...
            if (!empty($check_customer)) {
                ...
                if (!empty($results)) {
                    $json['status'] = 'success';
                    $json['message'] = iconv("UTF-8","UTF-8//IGNORE",$this->language->get('text_add_wishlist'));
                    ...
                    $json['status'] = 'failure';
                    $json['message'] = iconv("UTF-8","UTF-8//IGNORE",$this->language->get('text_fail_wishlist'));
                    $count_results = $this->model_webservices_wishlist->getWishlist($this->session->data['customer_id']);
                    $json['wishlist_count'] = count($count_results);
                }
            }
    ...
    public function appRemoveWishlist() {
        ...
        if (!empty($this->request->post['email']) && $this->request->post['product_id']) {
            ...
            if (!empty($check_customer)) {
                ...
                $json['status'] = 'success';
                $json['message'] = iconv("UTF-8","UTF-8//IGNORE",$this->language->get('app_text_wishlist_item_removed'));
                $count_results = $this->model_webservices_wishlist->getWishlist($this->session->data['customer_id']);
                $json['wishlist_count'] = count($count_results);
            }
        } else {
                    ...
                    foreach ($results as $result) {
                        ...
                        if ($product_info) {
                            if ($product_info['image']) {
                                $image = $this->model_webservices_image->resize($this->imageUrlEncoding($product_info['image']), $width, $height);
                            } else {
                                $image = false;
                            }
