...
//            $spin_win_response['wheel_display_interval'] = $config['display_interval'];
            ...
            if ($info_data['status'] == 1) {
                if ($info_data['cms_row_id'] == '0') {
                    $data[] = array(
                        ...
                        'link' => html_entity_decode(str_replace("amp;", "", $this->url->link('information/contact&lang='.$this->request->post['iso_code'], 'app=1','true')))
                    );
                } else {
                    $data[] = array(
                        'name' => html_entity_decode(str_replace("amp;", "", $info_data['title'])),
                        'link' => html_entity_decode(str_replace("amp;", "", $this->url->link('information/information', 'information_id=' . $info_data['link_to'] . '&app=1&lang='.$this->request->post['iso_code'],'true')))
                    );
                }
            }
        }
        ...
        if (isset($slideshow_data['status']) && $slideshow_data['status'] == 1) {
            ...
            foreach ($results as $result) {
                ...
                if (is_file(DIR_IMAGE . $result['image'])) {
                    ...
                    $image = $this->model_webservices_image->resize($this->imageUrlEncoding($result['image']), $width, $height);
                    if (!file_exists(DIR_IMAGE . $result['image'])) {
                        $image = $this->model_webservices_image->resize($this->imageUrlEncoding('placeholder.png'), $width, $height);
                    }
                    //Check if webp image exists
                    $info = pathinfo($image);
                    if (isset($info['extension'])) {
    ...
    //Function definition to get Best Seller products
    ...
    public function getCurrencies() {
        ...
        if (isset($this->session->data['currency']) && !empty($this->session->data['currency'])) {
            ...
            $data['default_currency_id'] = $this->session->data['currency'];
        } else {
        ...
        foreach ($data['lang_list'] as $key => $val) {
            if(file_exists(DIR_LANGUAGE . $val["directory"] . "/webservices/config.php")) {
                ...
                if(file_exists(DIR_LANGUAGE . $val["iso_code"] . "/webservices/config.php")) {
                    ...
                    $langdata[] = array("iso_code" => $val["iso_code"], "timestamp" => filemtime(DIR_LANGUAGE."english/webservices/config.php"));
                }
            }
        }
        return $langdata;
    ...
    public function appSpinWin() {
        ...
        $this->load->model('setting/setting');
        $this->load->model('module/spin_win');
        ...
        if (isset($data['setting']['spinwin_selectpages'])) {
            foreach ($data['setting']['spinwin_selectpages'] as $key => $value) {
                $data['select_pages'][] = $this->model_module_spin_win->getLayoutRoutes($value);
            }
        }
        $data['spin_data'] = $this->model_module_spin_win->getOffer('spinwin', $storeId);
        $this->load->language('module/spin_win');
