...
class ControllerWebservicesOneSixMyAccount extends Controller {
    ...
    public function appForgotPassword() {
        ...
        if (isset($this->request->post['email']) && $this->forgot_validate()) {
            ...
            $message .= $password;
            try {
                if (version_compare(VERSION, '2.0.2.1', '<')) {
                ...
                $data['status'] = 'failure';
                $data['message'] = $this->language->get('text_error_email');
                $this->response->addHeader('Access-Control-Allow-Methods: GET, PUT, POST, DELETE, OPTIONS');
                $this->response->addHeader('Access-Control-Max-Age: 1000');
                $this->response->addHeader('Access-Control-Allow-Headers: Content-Type, Authorization, X-Requested-With');
                $this->response->addHeader('Content-Type: application/json');
            ...
            $data['status'] = 'failure';
            $data['message'] = iconv("UTF-8","UTF-8//IGNORE",$this->error);
        }
        ...
        $this->load->model('account/customer');
            if (isset($this->request->post['mobile_number'])) {
                ...
                if (version_compare(VERSION, '2.0.1.0', '>=')) {
                    ...
                    if ($login_info && ($login_info['total'] > $this->config->get('config_login_attempts')) && strtotime('-1 hour') < strtotime($login_info['date_modified'])) {
                        $data['login_user'] = array('status' => 'failure', 'message' => $this->language->get('error_attempts'), 'customer_id' => '0');
                    }
                }
                if ($customer_info && !$customer_info['status']) {
                    $data['login_user'] = array('status' => 'failure', 'message' => $this->language->get('error_approved'), 'customer_id' => '0');
                }
                if (!$data['login_user']) {
//                    if (!($this->customer->login($this->request->post['email']))) {
                    if (false) {
                        $data['login_user'] = array('status' => 'failure', 'message' => $this->language->get('error_login'), 'customer_id' => '0');
                        if (version_compare(VERSION, '2.0.1.0', '>=')) {
                        ...
                        $data['login_user'] = array('status' => 'success', 'message' => $this->language->get('text_login_success'), 'customer_id' => $customer_info['customer_id'], 'wishlist_count' => count($results),'email' => $this->request->post['email_id'] ,'session_data' => $this->session->getId(), 'cart_count' => $cart_data_count);
                        if (version_compare(VERSION, '2.0.1.0', '>=')) {
...
//                    if (!($this->customer->login($this->request->post['email']))) {
        ...
        if (!empty($this->request->post['email'])) {
            ...
            foreach ($results as $result) {
                ...
                foreach ($products as $product) {
                    ...
                    $image = $this->model_webservices_image->resize($this->imageUrlEncoding($product_info['image']), 200, 200);
                    $json['products'][] = array(
        ...
        if (!empty($email) && $this->validatePassword()) {
            if($this->customer->login($email, $this->request->post['password'])){
                ...
                if (!empty($check_customer)) {
                    ...
                    $data['status'] = 'failure';
                    $data['message'] = iconv("UTF-8","UTF-8//IGNORE",$this->language->get('error_email_id'));
                }
            }else{
                $data['status'] = 'failure';
            ...
            if (isset($this->error['password'])) {
                $data['message'] = iconv("UTF-8","UTF-8//IGNORE",$this->error['password']);
            } else
            if (isset($this->error['confirm'])) {
                $data['message'] = iconv("UTF-8","UTF-8//IGNORE",$this->error['confirm']);
            } else {
	...
	$this->load->language('webservice/config');
        $this->load->language('account/register');
    ...
    public function appGetRegions() {
        ...
        if(!isset($data['zipcode_required'])){
    ...
    public function appGuestRegistration() {
        ...
        if (isset($this->request->post['email'])) {
            ...
            if (isset($this->request->post) && $this->validate()) {
                ...
                $data['status'] = 'failure';
                $data['message'] = iconv("UTF-8","UTF-8//IGNORE",$this->error['email']);    
            }
        } else {
            $data['status'] = 'failure';
    ...
    protected function validateAddress() {
        ...
        if ((utf8_strlen($this->request->post['shipping_address']['firstname']) < 1) || (utf8_strlen($this->request->post['shipping_address']['firstname']) > 32)) {
                $this->error['address'] = $this->language->get('error_firstname');
        }
        ...
        elseif ((utf8_strlen($this->request->post['shipping_address']['lastname']) < 1) || (utf8_strlen($this->request->post['shipping_address']['lastname']) > 32)) {
                $this->error['address'] = $this->language->get('error_lastname');
        }
        ...
        elseif ((utf8_strlen($this->request->post['shipping_address']['city']) < 2) || (utf8_strlen($this->request->post['shipping_address']['city']) > 128)) {
                $this->error['address'] = $this->language->get('error_city');
        }
        ...
        elseif ($country_info && $country_info['postcode_required'] && (utf8_strlen($this->request->post['shipping_address']['postcode']) < 2 || utf8_strlen($this->request->post['shipping_address']['postcode']) > 10)) {
                $this->error['address'] = $this->language->get('error_postcode');
        }

        elseif ($this->request->post['shipping_address']['country'] == '') {
                $this->error['address'] = $this->language->get('error_country');
        }

//        elseif (!isset($this->request->post['shipping_address']['state']) || $this->request->post['shipping_address']['state'] == '') {
//                $this->error['address'] = $this->language->get('error_zone');
//        }
//        
