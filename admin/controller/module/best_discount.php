...
class ControllerModuleBestDiscount extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->model_extension_module->addModule('best_discount', $this->request->post);
			} else {
		...
		$data['entry_discount_closed'] = $this->language->get('entry_discount_closed');
		...
		$data['label_opening'] = $this->language->get('label_opening');
		$data['label_upcoming'] = $this->language->get('label_upcoming');
		$data['label_closed'] = $this->language->get('label_closed');
        ...
        $data['column_discount_status'] = $this->language->get('column_discount_status');		
		...
		if (isset($this->error['height'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/best_discount', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('module/best_discount', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('module/best_discount', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				'text' => $this->language->get('text_edit_discount'),
				'href' => $this->url->link('module/best_discount/update', 'token=' . $this->session->data['token'] . '&product_id=' . $result['product_id'] . $url, 'SSL')
			);
		...
		$data['sort_name'] = $this->url->link('module/best_discount', 'token=' . $this->session->data['token'] . '&sort=pd.name' . $url, 'SSL');
		$data['sort_model'] = $this->url->link('module/best_discount', 'token=' . $this->session->data['token'] . '&sort=p.model' . $url, 'SSL');
		$data['sort_price'] = $this->url->link('module/best_discount', 'token=' . $this->session->data['token'] . '&sort=p.price' . $url, 'SSL');
		$data['sort_quantity'] = $this->url->link('module/best_discount', 'token=' . $this->session->data['token'] . '&sort=p.quantity' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('module/best_discount', 'token=' . $this->session->data['token'] . '&sort=p.status' . $url, 'SSL');
		$data['sort_order'] = $this->url->link('module/best_discount', 'token=' . $this->session->data['token'] . '&sort=p.sort_order' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('module/best_discount', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		if (isset($this->request->post['best_discount_product'])) {
			$discount_products = $this->request->post['best_discount_product'];
		} elseif (!empty($module_info)) {
			$discount_products = $module_info['best_discount_product'];
		} else {
			$discount_products = array();
		}
		
		$data['discount_products'] = array();
		...
		if (isset($this->request->post['label_setting'])) {
			$data['label_setting'] = $this->request->post['label_setting'];
		} elseif (!empty($module_info)) {
			$data['label_setting'] = $module_info['label_setting'];
		} else {
			$data['label_setting'] = array();
		}
		...
		$this->response->setOutput($this->load->view('module/best_discount.tpl', $data));
	}
	...
	public function getProductDiscounts(){
		...
		if($this->request->get['product_id']){
			...
			$json['output'] = $this->load->view('module/best_discount_special.tpl', $data);
		}
	...
	public function saveProductDiscounts(){
		if($this->request->get['product_id']){
			...
			$this->model_module_best_discount->editProductDiscounts($product_id,$product_discounts);
				
			$json['success'] = $this->language->get('entry_save_success');
			
			
			$json['redirect'] = $this->url->link('module/best_discount&token=' . $this->session->data['token'], 'SSL');
		}
		$this->response->setOutput(json_encode($json));
	}
	
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/best_discount')) {
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			foreach ($results as $result) {
				...
				foreach ($product_discounts  as $product_discount) {
					if ($product_discount['date_end'] == '0000-00-00' || $product_discount['date_end'] >= date('Y-m-d')) {
						$discount = $product_discount['price'];
