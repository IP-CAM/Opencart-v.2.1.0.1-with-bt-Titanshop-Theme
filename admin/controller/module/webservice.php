...
class ControllerModuleWebService extends Controller {

    private $error = array();

    public function index() {
        $this->load->language('module/webservice');
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            $this->response->redirect($this->url->link('module/webservice', 'token=' . $this->session->data['token'], 'SSL'));
        }
...
$data['text_both'] = $this->language->get('text_both');
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('module/webservice', 'token=' . $this->session->data['token'], 'SSL')
        );


        $data['action'] = $this->url->link('module/webservice', 'token=' . $this->session->data['token'], 'SSL');
        ...
        $data['sendNotificationUrl'] = str_replace('amp;', '', $this->url->link('module/webservice/sendNotification', 'token=' . $this->session->data['token'], 'SSL'));
        ...
        /*end:changes done by aayushi on 1st May 2020 to add preview changes*/
	$this->response->setOutput($this->load->view('module/webservice.tpl', $data).$this->load->view('module/general_settings_preview.tpl', $general_settings_data));
    }
    public function deleteLayout() {
        if(isset($this->request->get['layout_id'])) {
            ...
            $this->response->redirect($this->url->link('module/webservice', 'tab=home&token=' . $this->session->data['token'], 'SSL'));
        } else {
            $this->response->redirect($this->url->link('module/webservice', 'token=' . $this->session->data['token'], 'SSL'));
        }
    }
                ...
                foreach ($productInfoData as $productInfo) {
                    ...
                    if (isset($productInfo['special'])) {
                        if($productInfo['price']>0) {
                            $discount_price = $this->currency->format($productInfo['special'], $this->config->get('config_currency'));
                            $act_price = $productInfo['price'];
                            $disc_price = $productInfo['special'];
                            ...
                            $discount_price = $this->currency->format($productInfo['price'], $this->config->get('config_currency'));
                    ...
                    $data['products'][] = array(
                        ...
                        'price' => $this->currency->format($productInfo['price'], $this->config->get('config_currency')),
                        'src' => $image,
    ...
    public function layout() {
        ...
        if($this->request->get['layout_id']) {
            ...
            $this->response->redirect($this->url->link('module/webservice', '', 'SSL'));
        }
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('module/webservice/layout', 'token=' . $this->session->data['token'], 'SSL')
        );
        ...
        $data['column_left'] = $this->load->controller('common/column_left');
        $data['footer'] = $this->load->controller('common/footer');
        $data['cancel'] = $this->url->link('module/webservice', 'token=' . $this->session->data['token'], 'SSL');
        $this->response->setOutput($this->load->view('module/webservice_layout.tpl',$data));
    }

    protected function validate() {


        if (!$this->user->hasPermission('modify', 'module/webservice')) {
            $this->error['warning'] = $this->language->get('error_permission');
        }
        
        if (isset($this->request->post['webservice']['webservice_chat_status'])&& $this->request->post['webservice']['webservice_chat_status']==1 && $this->request->post['webservice']['chat_key']=="") {
            $this->error['chat_key'] = $this->language->get('error_chat_key');
        } 
        if (isset($this->request->post['webservice']['whatsapp_chat_status'])&& $this->request->post['webservice']['whatsapp_chat_status']==1 && $this->request->post['webservice']['whatsapp_phone_number']=="") {
            $this->error['whatsapp_number'] = $this->language->get('error_whatsapp_number2');
        } 
        if (isset($this->request->post['webservice']['homepage_layout'])&& $this->request->post['webservice']['homepage_layout']==0) {
            $this->error['homepage_layout'] = $this->language->get('error_homepage_layout');
        } 
//        $phone_number = explode("-",$this->request->post['webservice']['whatsapp_phone_number']);
//        
//        if (!isset($phone_number[1]))  {
//            $this->error['whatsapp_number'] = $this->language->get('error_whatsapp_number1');
//        }
//        else if (isset($this->request->post['webservice']['whatsapp_chat_status'])&& $this->request->post['webservice']['whatsapp_chat_status']==1 && preg_match ("/[^0-9]/",$phone_number[1])) {
//            $this->error['whatsapp_number'] = $this->language->get('error_whatsapp_number2');
//        }
        if (!$this->request->post['webservice']['app_button_color']) {
            $this->error['app_button_color'] = $this->language->get('error_app_button_color');
        }
        if (!$this->request->post['webservice']['app_theme_color']) {
            $this->error['app_theme_color'] = $this->language->get('error_app_theme_color');
        }
        if (!$this->request->post['webservice']['app_button_text_color']) {
            $this->error['app_button_text_color'] = $this->language->get('error_app_button_text_color');
        }
        if (!$this->request->post['webservice']['app_background_color']) {
            $this->error['app_background_color'] = $this->language->get('error_app_background_color');
        }
        if (isset($this->request->post['webservice']['cod_status'])&& $this->request->post['webservice']['cod_status']==1 && $this->request->post['webservice']['cod_name']=="") {
            $this->error['cod_name'] = $this->language->get('error_cod_name');
        }
        if (isset($this->request->post['webservice']['paypal_status'])&& $this->request->post['webservice']['paypal_status']==1 && $this->request->post['webservice']['paypal_name']=="") {
            $this->error['paypal_name'] = $this->language->get('error_paypal_name');
        }
        if (isset($this->request->post['webservice']['paypal_mode'])&& $this->request->post['webservice']['paypal_mode']==1 && $this->request->post['webservice']['paypal_clientid']=="") {
            $this->error['paypal_clientid'] = $this->language->get('error_paypal_clientid');
        }
...
//        $this->db->query("ALTER TABLE `" . DB_PREFIX . "kbb_components_heading` CHANGE `heading` `heading` VARCHAR(64) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL;");
        ...
        if (VERSION < '2.0.1.0') {
            ...
            $this->model_extension_event->addEvent('mailchimp_automation', 'post.order.add', 'webservices/firebase/on_order_add');
            $this->model_extension_event->addEvent('mailchimp_automation', 'post.order.history.add', 'webservices/firebase/on_order_update');
        }
    ...
    public function uninstall() {
        ...
        if (VERSION < '2.0.1.0') {
            ...
            $this->load->model('extension/event');
            $this->model_extension_event->deleteEvent('mailchimp_automation');
        }
