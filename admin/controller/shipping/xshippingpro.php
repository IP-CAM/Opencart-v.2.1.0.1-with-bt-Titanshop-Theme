...
class ControllerShippingXshippingpro extends Controller {
    ...
    public function __construct($registry) {
        ...
        $this->events[] = array(
            ...
            'action'  => 'shipping/xshippingpro/onOrderEmail'
        );
    }
    ...
    public function index() {
       ...
       if (isset($this->error['warning'])) {
        ...
        $data['export'] = $this->ocm->url->link('shipping/xshippingpro/export', '', true);
        ...
        $this->latencyImport();
        $data['method_data'] = $this->model_shipping_xshippingpro->getData();
    ...
    public function quick_save() {
       ...
       if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
           ...
           if(isset($this->request->post[$this->meta['name']]) && isset($this->request->post['tab_id'])) {
               ...
               $this->model_shipping_xshippingpro->addData($save);
               $json['success'] = true;
           } else {
    ...
    public function delete() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            if($this->request->post['tab_id']) {
                $this->model_shipping_xshippingpro->deleteData($this->request->post['tab_id']);
                $json['success'] = true;
            }
            else {
    ...
    public function export(){
        ...
        $this->load->model($this->ext_path);
        $export['method_data'] = $this->model_shipping_xshippingpro->getData();
    ...
    public function exportMethodRangeRows($tab_id) {
        $this->load->model($this->ext_path);
        $method_row = $this->model_shipping_xshippingpro->getDataByTabId($tab_id);
    ...
    public function import() {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST' && is_uploaded_file($this->request->files['file_import']['tmp_name']) && file_exists($this->request->files['file_import']['tmp_name'])) {
            ...
            if ($import_data) {
                $import_data = $this->model_shipping_xshippingpro->getUnCompressedData($import_data);
                if (is_array($import_data) && (isset($import_data['method_data']) || isset($import_data['xshippingpro']))) {
                    ...
                    foreach($method_data as $single) {
                        ...
                        $this->model_shipping_xshippingpro->addData($single);
                    }
                    if ($xshippingpro) {
    ...
    private function getRanges($method_data, $data, $no_of_tab) {
        ...
        $products = $this->model_shipping_xshippingpro->getBatchProductName($product_ids);
        $table_body = '';
    ...
    public function getProducts() {
        ...
        $json = $this->model_shipping_xshippingpro->getProducts($filter_data);
        $this->response->addHeader('Content-Type: application/json');
        $this->response->setOutput(json_encode($json)); 
    }
    ...
    public function install() {
        $this->load->model($this->ext_path);
        $this->model_shipping_xshippingpro->addDBTables();
        $this->ocm->util->addEvents($this->events);
    }

    public function uninstall() {
        $this->load->model($this->ext_path);
        $this->model_shipping_xshippingpro->removeDBTables();
