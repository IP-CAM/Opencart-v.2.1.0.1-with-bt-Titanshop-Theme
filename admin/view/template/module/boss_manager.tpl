			...
			<div class="panel-body">
				<form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-manufacturer" class="form-horizontal">
					...
					<ul class="nav nav-tabs">
						...
						<li><a data-toggle="tab" href="#sampledata"><?php echo 'Data Sample'; ?></a></li>
					</ul>
					...
					<div class="tab-content">
							...
							<div class="container-fluid">
								...
								<div class="panel-body">
									<table class="table table-striped table-bordered table-hover">
										<?php $dataModule = array("bestseller"=>"Best Seller","category"=>"Category","featured"=>"Featured","boss_manufacturer"=>"Boss Manufacturer","boss_zoom"=>"Boss Zoom","boss_alphabet"=>"Boss Alphabet","boss_featured"=>"Boss Featured","boss_tagcloud"=>"Boss Tagcloud","boss_quickshop"=>"Boss Quickshop","boss_revolutionslider"=>"Boss Revolution Slider","boss_menucategory"=>"Boss Menu Category","boss_carousel"=>"Boss Carousel","boss_procate"=>"Boss Product Category","boss_newmegamenu"=>"Boss Mega Menu","newslettersubscribe"=>"Newsletter","best_discount"=>"Best Discount","filter"=>"Filter","html"=>"HTML Content","boss_blog"=>"Boss Blog"); ?>
										<?php foreach($dataModule as $code => $name){ ?>
										<tr><td><label class="form-control"><?php echo 'Module '.$name; ?></label></td>
						...
						<div id="optionsetting" class="tab-pane active">
							<table class="table table-striped table-bordered table-hover">
								...
								<tr><td><label class="control-label"><span data-toggle="tooltip" title="" data-original-title="Disable/Enable button quickview product" class="help"><?php echo 'Quick View:'; ?></span></label></td><td class="left">
								<?php echo $btform -> makeSelectHTML($arrstatus, 'boss_manager[option][quick_view]','',isset($option['quick_view']) ? $option['quick_view'] : false); ?></td></tr>
								<!--<tr><td><label class="control-label"><span data-toggle="tooltip" title="" data-original-title="Disable/Enable responsive of the page" class="help"><?php echo 'Responsive:'; ?></span></label></td><td class="left">
								<?php echo $btform -> makeSelectHTML($arrstatus, 'boss_manager[option][responsive]','',isset($option['responsive']) ? $option['responsive'] : false); ?></td></tr>-->
								<tr><td><label class="control-label"><?php echo 'Show Refine Search:'; ?></label></td><td class="left">
								...
								<tr><td><label class="control-label"><?php echo 'Product Grid View:'; ?></label></td><td class="left">
								...
								<tr><td><label class="control-label"><?php echo 'Show loading (load page):'; ?></label></td><td class="left">
								<?php echo $btform -> makeSelectHTML($arrstatus, 'boss_manager[other][show_loading]','',isset($other['show_loading']) ? $other['show_loading'] : false); ?></td></tr>
							</table>
						</div>
						<!-- End #option setting-->
						...
						<div id="headersetting" class="tab-pane">
							...
							<td class="left"><table class="table table-striped table-bordered table-hover">
								...
								<tr><td>
								<?php echo $btform -> radioField('boss_manager[layout][h_mode_css]','hboxed',isset($layout['h_mode_css'])?$layout['h_mode_css']:'','Boxed'); ?></td><td> - Box Width: </td><td><?php echo $btform -> textField('boss_manager[layout][h_box_width]',isset($layout['h_box_width']) ? $layout['h_box_width'] : '1200',10); ?></td><td>px</td></tr></table>
							</td>
							</tr>
							</table>
						</div>
							...
							<div class="col-sm-10">
							 <div class="tab-content">
								...
								<div id="tab_footer_1" class="tab-pane">
									<table class="table table-striped table-bordered table-hover">
										...
										<tr>
											...
											<input type="hidden" name="boss_manager_footer_about[image_link]" value="<?php echo $footer_about['image_link']; ?>" id="input-aboutimage" />
											</td>	
										</tr>
										<tr>
									...
									<div id="tab-aboutus-<?php echo $language['language_id']; ?>" class="tab-pane">
										...
										<tbody>
											<tr>
												...
												<td>
													<input type="text" name="boss_manager_footer_about[about_title][<?php echo $language['language_id']; ?>]" value="<?php echo isset($footer_about['about_title'][$language['language_id']])?$footer_about['about_title'][$language['language_id']]:'About Us Title'; ?>">
												</td>
											</tr>
											<tr>
								...
								<div id="tab_footer_2" class="tab-pane">
									...
									<ul id="contact_language" class="nav nav-tabs">
										<?php foreach ($languages as $language) { ?>
							...
							<div class="form-group">
								...
								<div class="col-sm-10">
								  <select name="boss_manager[color]" id="input-status" class="form-control large" onchange="changeTemplate(this.value);">									
									<option value="1" <?php if($color==1)echo 'selected="selected"';  ?>>Blue(default)</option>
									<option value="2" <?php if($color==2)echo 'selected="selected"';  ?>>Red</option>
									<option value="3" <?php if($color==3)echo 'selected="selected"';  ?>>Green</option>
									<option value="4" <?php if($color==4)echo 'selected="selected"';  ?>>Violet</option>
									<option value="5" <?php if($color==5)echo 'selected="selected"';  ?>>Orange</option>
									<option value="6" <?php if($color==6)echo 'selected="selected"';  ?>>Pink</option>
									</select>
								</div>
							</div>
								  ...
								  <tbody class="boss_content" id="custom_color">
									...
									<?php foreach($objXML->customs->item as $xml){ ?>
										<tr id="custom_row<?php echo $custom_row; ?>">
											...
											<td class="text-left"><input type="text" name="custom[<?php echo $custom_row; ?>][text]" value="<?php echo $xml->text; ?>" class="form-control" /></td>
											<td class="text-left">
											<textarea name="custom[<?php echo $custom_row; ?>][class]"  class="form-control" ><?php echo $xml->class; ?></textarea></td>
											<td class="text-left"><select name="custom[<?php echo $custom_row; ?>][style]" class="form-control">
											...
											<td class="text-left"><input type="text" name="custom[<?php echo $custom_row; ?>][value]" id="id_custom<?php echo $custom_row; ?>" value="<?php echo $xml->value; ?>" class="form-control" /></td>
											<td class="text-left"><select name="custom[<?php echo $custom_row; ?>][important]" class="form-control">
...
<script type="text/javascript"><!--
<?php foreach ($languages as $language) { ?>
$('#bt_about_content<?php echo $language['language_id']; ?>,#bt_powered<?php echo $language['language_id']; ?>,#bt_contact_content<?php echo $language['language_id']; ?>').summernote({height: 300});
<?php } ?>
$('#about_language a:first').tab('show');
$('#contact_language a:first').tab('show');
...
function addCustom() {
	...
	html += '<td class="text-left"><input type="text" name="custom[' + custom_row + '][text]" value="" class="form-control" /></td>';
	html += '<td class="text-left"><textarea name="custom[' + custom_row + '][class]"  class="form-control" ></textarea></td>';
	html += '<td class="text-left"><select name="custom[' + custom_row + '][style]" class="form-control">';
	...
	html += ' <td class="text-left"><input type="text" name="custom[' + custom_row + '][value]" value="" id="id_custom' + custom_row + '" class="form-control" /></td>';
	html+= ' <td class="text-left"><select name="custom[' + custom_row + '][important]" class="form-control"> ';
