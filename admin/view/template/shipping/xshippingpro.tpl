...
<div id="content">
    <div class="page-header">
      ...
      <div class="pull-right float-right">
        <button type="button" id="save_form" form="form-ocm" value="save" name="action" data-toggle="tooltip" title="<?php echo $button_save; ?>" class="btn btn-primary"><i class="fa fas fa-save"></i></button>
        <a id="quick_save" onclick="return false;" data-toggle="tooltip" title="<?php echo $button_save_continue; ?>" id="quick_save" class="btn btn-info"><i class="fa fas fa-clipboard fa-window-restore"></i></a>&nbsp;
          ...
          <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-ocm" class="form-horizontal">
            ...
            <div class="row">
                <div class="col-sm-3">
                  ...
                  <button class="btn btn-success add-new" data-toggle="tooltip" form="form-ocm" type="button"  data-placement="bottom"  data-original-title="<?php echo $text_add_new_method; ?>"><i class="fa fa-plus"></i></button>
                </div>
                <div class="col-sm-9">
...
var unit_row ='<tr rel="{index}">'; 
    ...
    unit_row += '    <td class="text-left ocm-hide range"><select name="xshippingpro[ranges][{index}][partial]"><option value="0"><?php echo $text_no; ?></option><option value="1"><?php echo $text_yes; ?></option></select></td>';
    ...
    unit_row += '    <td class="text-right"><a class="btn btn-danger remove-row"><?php echo $text_remove; ?></a></td>';
    unit_row += '</tr>';
...
function toggleNoRanges() {
    if ($('#ocm-method-' + current_tab).find('.ocm-range-container tbody tr[rel]').length == 0) {
        $('#ocm-method-' + current_tab).find('.ocm-range-container tbody').append('<tr class="no-row"><td colspan="6"><?php echo $text_no_unit_row; ?></td></tr>');
    } else {
...
$(document).ready(function () {
    ...
    $('#quick_save, #save_form').click(function() {
        ...
        ocm.action.grabData().onEnd = function () {
            if (is_redirect) {
                location = "<?php echo $cancel; ?>".replace(/&amp;/g, '&');
            }
        };
