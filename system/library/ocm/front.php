...
final class Front {
    ...
    public function parseShortcode($str, $offset = 0) {
        // latency version
        if (is_array($str)) {
        ...
        $return = array();
        $rex = '/\[(\w+)\s?([^]]*)\](.*?)\[\/\w+\]/m';
        if (preg_match_all($rex, $str, $matches, PREG_SET_ORDER, $offset)) {
            foreach ($matches as $match) {
    ...
    */
    public function applyShortcode(&$str, $offset = 0) {
        if ($str && $shortcodes = $this->parseShortcode($str, $offset)) {
